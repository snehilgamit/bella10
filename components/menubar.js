                    ...
                    <div className='py-3.5 mx-2 relative border-b  border-orange-200'>
                        ...
                        <div className='opacity-0 hidden overflow-hidden shop_Submenu'>
                            <div className='m-2 mt-4'>
                                <div>
                                    {SubmenuItmes.map((el, index) => (
                                        <Link key={index} href={`/shop?category=${el}`} onClick={()=>{showMenu('hide')}} className='menuitmes_sidebar text-sm py-2 mx-2 relative border-t border-l border-r flex justify-between border-orange-200' style={{ 'border-bottom': el == 'all' ? '1px solid rgb(254 215 170)' : 'transparent' }}>
