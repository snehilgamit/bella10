...
const order = () => {
    ...
    const getOrder = async () => {
        const getData = await axios.post('/api/v1/admin/orders/getOrder', { email: router.query.email, orderID: slug });
        if (getData.data.status) {
            setData(getData.data);
        }
    }
    const cancelFunc = async () => {
        const req = await axios.post('/api/v1/admin/orders/cancel', { email:router.query.email ,orderID: slug });
        if(req.data.status){
            getOrder();
    ...
    const completeFunc = async () => {
        const req = await axios.post('/api/v1/admin/orders/complete', { email:router.query.email ,orderID: slug });
        if(req.data.status){
            getOrder();
            alert(req.data.message)
        }
