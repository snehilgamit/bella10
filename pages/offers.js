    ...
    <Head>
      ...
      <div className='flex m-5 min-h-[80vh] flex-col gap-2'>
        {offerArr.map((el, index) => (
          <div key={index} className='px-5 py-2 max-md:w-full w-[60%] text-lg text-red-500 brightness-105 bg-white border-2 border-black'><span className='text-black'>{index + 1})</span> {el}</div>
        ))}
